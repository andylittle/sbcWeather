...
class StationData(object):
    ...
    def parse_readings(raw):
        ...
        buf = raw[2:]
        data = dict()
        data['windSpeed'] = 0 #StationData._decode(buf[0:4], 0.1 * MILE_PER_KM) # mph
        data['windDir'] = 0 # StationData._decode(buf[6:8], 1.411764)  # compass deg
        data['outTemp'] = StationData._decode(buf[8:12], 0.01, True)  # degree_F
        data['long_term_rain'] = 0 # StationData._decode(buf[12:16], 0.01)  # inch
        ...
        data['outHumidity'] = StationData._decode(buf[24:28], 0.01, True)  # percent
        data['inHumidity'] = 0 #StationData._decode(buf[28:32], 0.1)  # percent
        data['day_of_year'] = 0 #StationData._decode(buf[32:36])
        data['minute_of_day'] =  0 #StationData._decode(buf[36:40])
        data['daily_rain'] = 0 #StationData._decode(buf[40:44], 0.01)  # inch
        data['wind_average'] = 0 # StationData._decode(buf[44:48], 0.1 * MILE_PER_KM)  # mph
        return data
